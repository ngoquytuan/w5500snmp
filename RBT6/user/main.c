...
void usart1Process(void)
{
	...
	//UART1 RX process
			if(u1out == ONTIME)
			{
				...
				//printf("UART1:%s\r\n",USART1_rx_data_buff);
				/*
				for(i=0;i<USART1_index;i++)
				{
					printf("%02X ",USART1_rx_data_buff[i]);
				}
				printf("\r\n");
				*/
				#endif
				u1_message_handle();
				//memset(USART1_rx_data_buff,100,0);
